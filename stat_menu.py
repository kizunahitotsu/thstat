...
def select_session_menu(init_info, database):
    ...
    while continue_flag:
        ...
        for i in range(2):
            is_selected = i == graph_option_idx
            graph_option_layout.append([sg.Radio(text=f'',
                                                 key=f'Radio {i + 1}',
                                                 group_id='radio_group',
                                                 default=is_selected,
                                                 enable_events=True)] +
        ...
        layout = [[sg.Text('Select a session to view statistics')],
                  [sg.Listbox(values=items, default_values=default_values,
                              ...
                              size=(40, 20), key='-SESSION-'), sg.Column(graph_option_layout)],
        ...
        if event in [SHOW_STAT_STR, REMOVE_SESSION_STR]:
            ...
            if len(selected_strs) != 0:
                ...
                if event == SHOW_STAT_STR:
                    stat_by_session.show_session_stat_menu(init_info, database, session_idx, graph_option)
...
def gameplay_session_creation_menu(init_info, database, session_idx):
    ...
    while continue_flag:
        stat_layouts = stat_by_session.create_session_text_statistics_layout(database, session_idx)
        ...
        if event == 'Submit':
            ...
            database.commit()
            success_dict = get_default_success_dict(config_stages)  # don't assume same outcome in diff. runs
